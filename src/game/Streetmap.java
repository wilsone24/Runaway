...
public class Streetmap extends PApplet {
    ...
    public void setup() {
        ...
        background=loadImage("images/MapaZombie.png");
        mainCharacter=loadImage("images/Carro.png");
        ...
        obstacle = loadImage("images/Obstaculoz.png");
        obstacle2 = loadImage("images/Obstaculoz2.png");
        imagePoints = loadImage("images/Gasolina.png");
        secondCharacter = loadImage("images/CpuCarro.png");
        first=loadImage("images/Primero.png");
        second=loadImage("images/Segundo.png");
        hollow=loadImage("images/SaltoZ.png");
    ...
    public void draw() {
        ...
        if (keyPressed == true && key == 'd') {
            mainCharacter = loadImage("images/Carro2.png");
            if (positionCharacter.x==280){// range of movement
           ...
           mainCharacter = loadImage("images/Carro3.png"); 
           if (positionCharacter.x==40){// // range of movement
        ...
        int temp=0;
        for (int i = 0; i < vidasNumero2.length; i++) {//convert lives from array to variable
            if (vidasNumero2[i]==1){
        ...
        if (lives<=0){
            LoseZombiev loseZv = new LoseZombiev();
            loseZv.runApp(); 
            this.dispose();
        }else{
            if (seconds>=80){
                if (positionCharacter2.y<positionCharacter.y){
                    LoseZombie22 loseZ2 = new LoseZombie22();
                    loseZ2.runApp(); 
                    this.dispose();
                }else{
                    WinZombies winz = new WinZombies();
                    winz.runApp();
                    this.dispose();
                }
            }
    ...
    public void repeatPosition(PVector element, int x){
        if (element.y>=height){// agregar Gasolina y zombies cada que pasen el largo  
            if (x==1){
    ...
    public void checkCollision(PVector playerPosition, PVector squarePosition, int playerSize, int squareSize, int m) {
        ...
        if(isColliding && !isArcherImmune()) {
            ...
            for (int i = 0; i < vidasNumero2.length; i++) {// Remove life of the array
                if (vidasNumero2[i]==1) {
                    vidasNumero2[i]=0;
                    break;
                }else{
